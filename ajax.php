...
class AuthorizationAjaxRequest extends AjaxRequest
{
    ...
    public function login()
    {
        ...
        if (empty($username)) {
            $this->setFieldError("username", "Введіть ім'я");
            return;
        }

        if (empty($password)) {
            $this->setFieldError("password", "Введіть пароль");
            return;
        }
        ...
        if (!$auth_result) {
            $this->setFieldError("password", "Невірний пароль чи логін");
            return;
        }

        $this->status = "ok";
        $this->setResponse("redirect", "tabs.php");
        $this->message = sprintf("Привіт, %s! доступ отримано", $username);
    }
